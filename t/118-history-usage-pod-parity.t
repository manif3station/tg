...
# for cli/history.pl. cli/history.pl has no --help of its own - the
# Usage string only ever prints when a leftover positional/unrecognized
# argument survives its own --since/--until parsing loop (TGT-122),
# triggered here via a genuine subprocess invocation with one stray arg.

sub read_source {
    open my $fh, '<', "$Bin/../cli/history.pl" or die $!;
    local $/;
    return <$fh>;
}
...
my $full_output = qx{$^X "$Bin/../cli/history.pl" leftover-positional 2>&1};
is( $? >> 8, 2, 'cli/history.pl with a leftover positional argument refuses with exit 2' ) or diag $full_output;
die "cli/history.pl produced no output\n" unless length $full_output;

my ($usage_output) = $full_output =~ /^(Usage:.*)$/m;
die "cli/history.pl produced no Usage: line\n" unless defined $usage_output;

my $source = read_source();

my ($synopsis) = $source =~ /=head1 SYNOPSIS\n\n(.*?)\n\n=head1/s;
die "SYNOPSIS section not found in cli/history.pl's POD\n" unless $synopsis;
