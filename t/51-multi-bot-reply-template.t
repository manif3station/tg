...
# --- board, not a private log) ---
{
    ...
    like(
        $out,
        qr/REPLY WITH: d2 tg\.reply --bot 1234\.\.\.sxyz --reply-to-message-id 42 4567 "\.\.\."/,
        'multi-bot REPLY WITH names the receiving bot by its masked token (TGT-086), both --bot and --reply-to-message-id leading (TGT-227/TGT-322 - the one position cli/reply.pl actually parses)'
    );
...
# --- single-bot mode (no bot_token given): REPLY WITH is unchanged ---
{
    ...
    like(
        $out,
        qr/REPLY WITH: d2 tg\.reply --reply-to-message-id 43 999 "\.\.\."\n/,
        'single-bot REPLY WITH is unchanged aside from --reply-to-message-id moving leading (TGT-322) - no --bot'
    );
