...
# for cli/whoami.pl. No --help of its own - the Usage string only ever
# prints on a leftover positional argument, checked before any storage
# resolution, so no D2TG_DB/mandatory env setup is needed.

sub read_source {
    open my $fh, '<', "$Bin/../cli/whoami.pl" or die $!;
    local $/;
    return <$fh>;
}
...
my $full_output = qx{$^X "$Bin/../cli/whoami.pl" leftover-positional 2>&1};
is( $? >> 8, 2, 'cli/whoami.pl with a leftover positional argument refuses with exit 2' ) or diag $full_output;
die "cli/whoami.pl produced no output\n" unless length $full_output;

my ($usage_output) = $full_output =~ /^(Usage:.*)$/m;
die "cli/whoami.pl produced no Usage: line\n" unless defined $usage_output;

my $source = read_source();

my ($synopsis) = $source =~ /=head1 SYNOPSIS\n\n(.*?)\n\n=head1/s;
die "SYNOPSIS section not found in cli/whoami.pl's POD\n" unless $synopsis;
