...
#
# TGT-290: $boundary is now generated by _generate_boundary (8 rounds of
# rand(65536) plus time) - both rand and time are overridden below (via
# CORE::GLOBAL, set up in a BEGIN block before D2TG::Telegram is compiled)
# so the exact boundary value is known and a colliding caption can be
# crafted deterministically. The mocked rand() returns the same fixed
# value on every call, so the 8 rounds collapse to 8 repetitions of the
# same 4-hex-digit chunk - still fully deterministic for this test.
