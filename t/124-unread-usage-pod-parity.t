...
# for cli/unread.pl. No --help of its own - the Usage string only ever
...
sub read_source {
    open my $fh, '<', "$Bin/../cli/unread.pl" or die $!;
    local $/;
    return <$fh>;
}
...
my $full_output = qx{$^X "$Bin/../cli/unread.pl" leftover-positional 2>&1};
is( $? >> 8, 2, 'cli/unread.pl with a leftover positional argument refuses with exit 2' ) or diag $full_output;
die "cli/unread.pl produced no output\n" unless length $full_output;

my ($usage_output) = $full_output =~ /^(Usage:.*)$/m;
die "cli/unread.pl produced no Usage: line\n" unless defined $usage_output;

my $source = read_source();

my ($synopsis) = $source =~ /=head1 SYNOPSIS\n\n(.*?)\n\n=head1/s;
die "SYNOPSIS section not found in cli/unread.pl's POD\n" unless $synopsis;
