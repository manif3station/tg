...
time-of-day component - a value like `2026-01-01T99:99:99` silently reached
the SQL range query and excluded real messages, the exact misleading "No
messages found." class those earlier fixes closed for other malformed
shapes. Now round-trips `HH:MM:SS` through `Time::Piece` too and refuses a
...
shape - the exact inconsistency TGT-211 already fixed for
`cli/attachment.pl`/`cli/retry-download.pl`/`cli/approve.pl`, but that
sweep's scope missed these 3 scripts. A caller supplying both a bad
`--db` alias and malformed positional args got an inconsistent
exit 1/storage-error instead of exit 2/`Usage:` from these 3 scripts,
unlike every already-fixed sibling command. Reordered to match TGT-211's
own established canonical order - a pure block-reorder, no new logic.
