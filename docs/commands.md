...
| Module | What it does |
| --- | --- |
| `D2TG::Config` | Reads `D2TG_TOKEN`/`D2TG_CHAT_ID`; startup guard; resolves `state/store.sqlite`'s path (or, for a resolved `--db`/`-d`/`D2TG_DB`/`TIRA_HOME` base_dir, `.tira/telegram.messages.db` and `.tira/attachments/` - TGT-081); `skill_version` reads `.env`'s installed VERSION (TGT-036); `masked_token` masks a token to its first/last 4 chars for safe display (TGT-045); `owner_name` reads `D2TG_OWNER` (TGT-079); `extract_db_flag`/`resolve_alias_dir`/`attachments_dir` resolve an optional `--db`/`-d`/`D2TG_DB` Developer Dashboard path alias (or a `TIRA_HOME` fallback, TGT-081) for both storage and attachments (TGT-051); `bot_groups` parses repeatable `--chat_id`/`--bot` CLI args, folding in `D2TG_CHAT_ID`/`D2TG_TOKEN` as an implicit trailing pair through the same grouping algorithm (TGT-049); `shift_flag_value` (TGT-072) is a shared helper backing `extract_db_flag`'s `--db`/`-d`, `bot_groups`'s `--chat_id` and (TGT-074) `--bot`, `cli/reply.pl`'s own `--db`, `cli/history.pl`'s `--since`/`--until`, and `D2TG::Reply::extract_bot_flag`'s `--bot` validation - one implementation instead of many independent hand-rolled copies; `lock_path` resolves `cli/poller.pl`'s single-instance lock file path, mirroring `state_db_path`/`attachments_dir`'s own base_dir/`.tira/` resolution exactly (`.tira/telegram.pid` for a resolved base_dir, TGT-087); `resolve_alias_dir_or_die` (TGT-172, found via a scheduled improvement hunt) wraps `resolve_alias_dir` in the eval/print-STDERR/exit(1) pattern that 11 of the 13 `cli/*.pl` scripts had each duplicated identically - a pure extraction, no behavior change; `_with_hard_timeout` (TGT-173, found via a scheduled improvement hunt) is a shared SIGALRM-based hard-timeout wrapper extracted after `D2TG::Telegram` (TGT-044) and `D2TG::Download` (TGT-126) each independently implemented the identical control flow - each call site passes its own exact pre-extraction die-message prefix, so behavior is unchanged; `require_existing_base_dir` (TGT-090) dies if the resolved base_dir doesn't already exist on disk - called by every `cli/*` script right after `resolve_alias_dir`/`resolve_alias_dir_or_die`, so a bogus `TIRA_HOME`/alias target is refused instead of silently `mkdir -p`'d into existence; `require_existing_base_dir_or_die` (TGT-230, found via a scheduled JOB-004 improvement hunt) wraps it in the identical eval/print-STDERR/exit(1) pattern `resolve_alias_dir_or_die`/`extract_db_flag_or_die` already established for their own wrapped calls - all 11 `cli/*.pl` scripts that call `require_existing_base_dir` had each duplicated the wrapper identically; a pure extraction, no behavior change; `resolve_self_exec_path` (TGT-094) checks whether a given basename (`poller.pl`) exists in a given bin directory, returning that fresh path if so and a supplied fallback otherwise - `cli/poller.pl`'s version-change restart uses this instead of blindly `exec()`ing the literal `$0` path captured at launch, so a running poller survives an install that renames its own entrypoint file out from under it; `is_transient_error` (TGT-097, widened by TGT-160) classifies an error message as transient (matches `/timed out/i`, `/status 5\d\d/`, or `/status 429\b/`) or not - a shared predicate now used by both `D2TG::Reply::format_send_error` (TGT-096) and `D2TG::Poller::run_once_safe` (TGT-097), avoiding duplicated regex. The `429` ("Too Many Requests") case (TGT-160, found via a scheduled hourly bug hunt) treats Telegram's own designed, expected, retryable rate-limit condition (a response body containing `parameters.retry_after`) the same as a 5xx/timeout - silently retried rather than logged as a `POLL ERROR` - without reading/honoring `parameters.retry_after` itself, which would require parsing the response body and is out of scope for this narrow classification fix. `heartbeat_path`/`write_heartbeat`/`heartbeat_age` (TGT-116) mirror `lock_path`'s own `.tira/` resolution exactly (`.tira/telegram.heartbeat` for a resolved base_dir) - the poller writes a heartbeat after each bot/chat pair's own poll cycle, unconditionally, atomically (temp file + `rename`, so a concurrent reader or a crash mid-write never observes a partial file), so `d2 tg.status` can distinguish "still genuinely cycling" from "alive but silently wedged". `is_expired_file_error` (TGT-104) classifies an error as Telegram's own shape for a permanently-gone `file_id` (`/file is no longer available/i` or `/wrong file_id/i`) - deliberately NOT "file is temporarily unavailable" (a Codex review caught an earlier draft treating that transient-sounding wording as permanent, when it can still succeed on a later retry); backs `cli/retry-download.pl`'s `RETRY EXPIRED` vs `RETRY FAILED` distinction. `extract_db_flag_or_die` (TGT-177, found via a scheduled improvement hunt) wraps `extract_db_flag` in the identical eval/print-STDERR/exit(1) pattern `resolve_alias_dir_or_die` already established for `resolve_alias_dir` - 10 `cli/*.pl` scripts (approve, attachment, history, poller, retry-download, send, status, text-only-replies, unread, whoami) had each duplicated it identically; a pure extraction, no behavior change. `cli/reply.pl` is unaffected, using its own separate leading-position-only `--db` extraction instead. `bot_groups` (TGT-202, found via a scheduled JOB-003 hourly bug hunt) now refuses (dies) when the env-folded D2TG_CHAT_ID/D2TG_TOKEN pair exactly duplicates an explicit CLI `--chat_id`/`--bot` pair across any group - previously it silently created two group entries sharing the same `(chat_id, bot token)` pair, which would make `cli/poller.pl`'s own `@pairs` construction poll that one bot token twice per cycle, racing its own `get_offset`/`set_offset` calls against itself; a genuinely distinct multi-bot/multi-chat configuration (including the same `chat_id` with two different tokens) is unaffected. `bot_groups` (TGT-213, found via a scheduled JOB-004 improvement hunt) also now refuses when the same bot token is configured under two *different* `chat_id` groups - TGT-202's own guard above is keyed on `(chat_id, token)` and never caught this, but the real offset-race hazard is keyed on the token alone (`D2TG::Store::get_offset`/`set_offset` key the shared offset row purely on `bot_key`, with no `chat_id` involved), so a token reused across chat_ids still produced the identical race, just uncaught by the narrower key; the refusal names the masked token and both conflicting `chat_id`s. `bot_groups` (TGT-218, found via a scheduled JOB-003 hourly bug hunt) also now validates a `--chat_id` value against Telegram's own canonical chat-id shape (`^-?\d+$`), reusing the identical check `require_chat_id_or_warn` (TGT-155/164) already enforces for `D2TG_CHAT_ID` - previously a whitespace-padded or non-digit `--chat_id` value was accepted silently, and since a mangled value can never string-eq match a real inbound `chat_id`, the owner was invisibly locked out forever (`cli/poller.pl`'s own TGT-164 re-validation only ever re-checked `D2TG_CHAT_ID` when set alongside CLI groups, never the CLI-declared value itself). `state_db_path`/`attachments_dir`/`lock_path`/`heartbeat_path`/`write_heartbeat`/`heartbeat_age`/`resolve_alias_dir`/`resolve_alias_dir_or_die`/`require_existing_base_dir`/`require_existing_base_dir_or_die`/`resolve_self_exec_path` (TGT-260, found via TGT-258's own module-decomposition survey) are now thin forwarders onto `D2TG::Config::Paths` - no behavior/signature change for any caller. `_developer_dashboard_paths` moved with them and got no forwarder (private, only ever called from within the same extracted cluster). |
...
| `D2TG::Telegram` | Raw HTTP Bot API client (`LWP::UserAgent`, no SDK, explicit 50s timeout - TGT-035/TGT-066, backed by a real SIGALRM-based hard timeout since a stuck TCP connect() was found to bypass it in production - TGT-044): `get_me`, `get_updates`, `get_file`, `file_download_url`, `send_message` (auto-split, optional `reply_to_message_id` - TGT-040), `send_voice` (multipart, optional `reply_to_message_id` - TGT-040), `send_photo`/`send_document` (TGT-103, multipart via a shared `_send_file` helper, optional `caption`/`reply_to_message_id`) - the outbound counterpart to `D2TG::Download`'s inbound media handling. `_send_file` escapes and sanitizes the local file's basename before inserting it into the multipart `Content-Disposition` header (TGT-125, found via a scheduled bug-hunt) - a literal double-quote previously corrupted the header, and a literal CR/LF could have injected an additional header line into the request; C0 control characters and DEL are stripped, then backslashes and quotes are escaped. `get_updates` now requests `allowed_updates` explicitly, defaulting to `[message, edited_message, message_reaction]` (`message_reaction` - TGT-143; `edited_message` - TGT-169, both live Telegram questions). `message_reaction` is genuinely opt-in - excluded from Telegram's own baseline default set even on a bot's first-ever call omitting `allowed_updates`; `edited_message` is not - that same baseline already includes it, and it only stopped reaching this project the moment `allowed_updates` was first narrowed to a fixed list at all (TGT-143). That baseline is a historical fact, not a live fallback: `getUpdates` retains whichever `allowed_updates` a bot last set rather than reverting to the baseline default if a later call omits the parameter, so omitting it now would not restore `edited_message` - it must be listed explicitly, as this default now does. Telegram's own docs warn that specifying `allowed_updates` at all restricts delivery to only the listed types, so every type this project already relies on is preserved alongside each newly-added one. `_send_file`'s `caption` field (TGT-162, found via a scheduled bug-hunt) now also conservatively strips the literal per-call multipart boundary string from itself before insertion - the same trust boundary (a user-supplied `cli/send.pl` argument) as `filename` above, but a narrower risk: caption sits in body content rather than a quoted header attribute, so it needs none of `filename`'s quote/backslash escaping or control-character stripping (CR/LF there is legitimate caption text, not a header-injection vector) - the one real risk was a caption embedding the boundary value in real delimiter syntax (`\r\n--$boundary`, not the bare value alone) prematurely terminating the multipart body. `reply_to_message_id`'s numeric validation (originally added by TGT-040/TGT-055) is shared across `send_message`/`send_voice`/`_send_file` via a private `_validate_reply_to_message_id($method, $reply_to_message_id)` helper (TGT-171, found via a scheduled improvement hunt - all 3 call sites had triplicated the same validation logic, only the method name in the die message differed, a pure extraction with no behavior change). `send_voice` and `_send_file` additionally shared the identical "validate, then if defined append a multipart form-data fragment to the body" 5-line block - extracted into `_append_reply_to_message_id_field($body_ref, $boundary, $method, $reply_to_message_id)` (TGT-182, found via a scheduled improvement hunt), taking the body by scalar ref since the caller's own `$body` must keep accumulating fragments after the call returns (the same by-reference approach `D2TG::Poller::_record_message_and_track_offset` already established for TGT-181). Pure extraction, no behavior change. |
| `D2TG::Poller` | `run_once` — one poll cycle: access-control gate (its `is_allowed`/`add_pending` calls pass the pair's own `bot_token` through as the store's `bot_key`, TGT-098 - so a multi-bot config's per-bot access scoping is enforced on the inbound side too, not just at `cli/approve`), text/voice/media event lines (sanitized to always be a single stdout line, even a multi-segment voice transcript - TGT-039; naming the message's own `message_id` - TGT-040; plus a `(replying to ... [msg #N]: ...)` suffix when the message is itself a reply, naming the original message's own id (TGT-041) and preferring our own stored message history over Telegram's bare payload - TGT-029/TGT-038), the `REPLY WITH` template (now including `--reply-to-message-id` - TGT-040), non-fatal error handling for voice/media, a specific error for files over Telegram's 20MB `getFile` limit (TGT-037); before a voice message's (potentially multi-minute, blocking) transcription attempt, prints a `NEW TG VOICE [chat_id] sender: transcribing... (this may take a few minutes)` notice so the watching agent notices immediately instead of the whole wait being silent (TGT-100, live user request). `run_once_safe` wraps it so a failure is retried after a short backoff instead of killing the poller (TGT-028); a known-transient failure (network timeout, 5xx status) retries silently with no STDERR output, while a genuinely unexpected/non-transient failure is still logged as `POLL ERROR` (TGT-097, via `D2TG::Config::is_transient_error`). A media download failure, when a store is present, is also queued via `D2TG::Store::record_failed_download` (TGT-104) for later retry - wrapped in its own `eval` (a Codex review finding: a locked/full SQLite database must not turn an already-non-fatal download error into a poll-cycle failure). A downloaded photo/document's real local path is never printed (TGT-133) - `_print_attachment_template` prints a `GET ATTACHMENT WITH: d2 tg.attachment <chat_id> <message_id>` instruction instead, and `record_message` receives the path separately via its own `local_path` argument, never folded into the printed/stored summary text. All 4 `record_message` call sites (text/voice/media/fallback) go through a private `_record_message_safe` eval-wrapper (TGT-132, found via an ad-hoc bug-hunt) matching `record_failed_download`'s own established pattern - a store write failure is logged non-fatally to STDERR instead of aborting `run_once`, which would otherwise make `run_once_safe` redeliver and reprint the entire batch next cycle. A forwarded message's sender name now also names the original author, not just the forwarder (TGT-142, a live question answered by Michael) - `_forward_origin_name` reads Telegram's own `forward_origin` field (all 4 `MessageOrigin` types: a real user by name never id, a privacy-restricted user's Telegram-supplied name only, or the originating chat/channel), applied to both the main sender line and `_reply_context_suffix`'s own `$original_sender` via a shared `_format_forwarded_sender($sender, $forward_origin)` helper (TGT-170, found via a scheduled improvement hunt - both call sites duplicated the same four-line formatting logic, only the variable name differed, a pure extraction with no behavior change, matching `_classify_store_error`'s own TGT-167 precedent); a non-forwarded message is unaffected. A `message_reaction` update (TGT-143, a live Telegram question) is now detected and printed as its own event, before any `message` handling - never recorded into the store, detection/printing only. Gated by the same `is_allowed` access-control check the message branch uses (TGT-151, found via a scheduled bug hunt: this branch originally ran entirely before that gate, so an unapproved, non-pending chat_id's reaction was printed unconditionally) - an unapproved chat_id's reaction is silently ignored, never added to the pending queue since a reaction isn't a first-contact event the way a message is. When the reaction is anonymous (made on behalf of a chat/channel rather than a user - Telegram's own `MessageReactionUpdated.user` field is optional) the sender name comes from `actor_chat.title`/`.username` instead of `_display_name` (TGT-154, found via a scheduled bug hunt: previously only `user` was ever read, so an anonymous channel reaction silently printed `sender: unknown` even though Telegram supplied the channel's real name via `actor_chat` - the same failure class TGT-142 already fixed once for `forward_origin`'s own chat/channel fields). `MessageReactionUpdated` reports the full current/previous reaction sets, not a single before/after pair, so they're diffed by a type+id key, not by emoji alone (a Codex review finding: `custom_emoji`/`paid` reactions carry no emoji glyph, so keying on emoji would collapse two different custom emojis into one bucket) - a key present in `new_reaction` but not `old_reaction` prints `NEW TG REACTION [chat_id] sender: <label> on message <id>`, one present in `old_reaction` but not `new_reaction` prints `REACTION REMOVED [chat_id] sender: <label> on message <id>` (the emoji glyph for a standard reaction, a plain description otherwise) - both sanitized before printing. Anonymous aggregate reaction counts (`message_reaction_count`) are out of scope. A `video` message (TGT-161, found via a scheduled bug hunt) is now recognized by `_media_kind` too - previously a video with no plain text and no recognized media kind (`_media_kind` only knew photo/document/voice) failed `run_once`'s own "next unless text or media_kind" guard and was silently dropped: not printed, not queued pending, not recorded, no stderr line, the poll offset still advancing past it. The existing generic fallback branch - which video always reaches, since there is no video-specific handling or download path at all, unlike photo/document/voice which only reach it when their own callback is omitted - already prints/records generically by media kind, so recognizing `video` there was enough; no other code change was needed. Video download support itself is a separate, larger decision and stays out of scope; `video_note`/`audio`/`animation`/`sticker` are the same failure class but are deliberately deferred to a follow-up ticket. The `is_allowed` calls in both the message and message_reaction branches, and the `add_pending` call in the message branch (TGT-165, found via a scheduled bug hunt), are now `eval`-wrapped the same way `_record_message_safe` already wraps `record_message` (TGT-132) - all three run against a `RaiseError=>1` DBI handle, so a locked/busy SQLite database used to make any of them die unwrapped, aborting the whole batch and (since `run_once_safe` preserves the pre-batch offset on error) causing the entire batch to be redelivered and reprinted next cycle; a store error at any of them is now logged to STDERR as `STORE ERROR [chat_id]: ...` and that one update is skipped non-fatally instead. `persist_offset_safe` (TGT-166, found via a direct follow-up sweep after TGT-165 for the same unwrapped-DBI-call pattern) is a new public function `cli/poller.pl`'s main loop calls instead of `$store->set_offset` directly - unlike `run_once`'s own calls, that call sat at the top level of the persistent poller script's main loop, outside `run_once_safe`'s own protection, so a locked database used to crash the entire poller process rather than just one poll cycle's batch; it now catches the error, classifies it into a short fixed reason (matching `_record_message_safe`'s own TGT-133-precedent classification, never echoing the raw exception), logs it to STDERR, and returns a false success flag (TGT-191, changed from always-void) so the poller keeps running; the caller (`cli/poller.pl`'s main loop) only advances its in-memory offset when this returns true (see TGT-191 below), so a failed persist means a later poll cycle retries persisting THIS SAME offset, not a newer one, and the corresponding `getUpdates` call is re-issued with the same still-unconfirmed-to-Telegram offset too - Telegram redelivers rather than discarding it. `_record_message_safe` and `persist_offset_safe` now share their error-classification logic via a private `_classify_store_error` helper (TGT-167, found via a scheduled improvement hunt - a duplication-removal refactor with no behavior change, since the same 4-line ternary was found duplicated byte-for-byte between the two functions). A new branch recognizes `update.edited_message` (TGT-169, a live Telegram question from Michael) - Telegram's own distinct update, generally sent when a message known to the bot in an allow-listed chat is edited - and prints a `NEW TG EDIT [chat_id] sender: <new text> (msg #N, edited)` line, gated by the same `is_allowed` check every other branch uses by chat_id (no `add_pending` - an edit isn't a first-contact event, matching `message_reaction`'s own precedent above). A text edit's new content is recorded via `record_message` so `d2 tg.history` reflects it; a caption/media-only edit (no text) is announced but deliberately not recorded, to avoid overwriting an already-correct history summary with nothing useful (a Codex review finding - recording those too is a narrower follow-up, out of this ticket's own scope). Deletion of an ordinary chat message cannot be detected at all - the Bot API has no update for that (a separate, business-connection-scoped `deleted_business_messages` update exists for an unrelated feature this project doesn't use, a Codex review correction to an earlier, too-absolute claim) - a hard platform limitation, not a gap in this implementation. `skill_version_check_safe` (TGT-175, a live production incident reported via the budget project) wraps `D2TG::Config::skill_version` for `cli/poller.pl`'s main-loop version-change check - a transient `.env` read failure during the skill's own self-update used to crash the entire poller process; now logged to STDERR non-fatally, returning `undef` so that cycle's version-change check is simply skipped and retried next cycle, matching `persist_offset_safe`'s own established degradation pattern. The poller's own startup call to `skill_version` (which still refuses to start loudly on a genuinely missing `.env`) is deliberately untouched. `_record_message_safe` (TGT-178, a design decision by Michael answering TGT-176's own message-loss investigation) now returns a true/false success flag instead of void - a genuine `record_message` failure (locked/busy/readonly database) used to be logged non-fatally while the offset still advanced past that update regardless, permanently losing it, since Telegram never redelivers an update once the offset has moved past it. `run_once` now tracks the first failing update's own `update_id` encountered while iterating the batch in order, across all 5 `_record_message_safe` call sites and caps the returned offset there instead of the batch's own full next offset, so Telegram redelivers that update (and everything after it in the same batch) next cycle; `record_message`'s own existing `PRIMARY KEY (chat_id, message_id)` upsert already made a redelivered message's store write idempotent, so no schema change was needed. To avoid a redelivered-but-already-recorded update being re-announced (a duplicate `NEW TG`/re-download/re-transcription), `run_once` checks `store->get_message($chat_id, $message_id)` before acting on a plain message and skips it entirely when already found - a lookup failure is treated as "not previously recorded" (proceed as normal), the same degrade-not-crash philosophy as `_record_message_safe` itself. `_record_message_and_track_offset` (TGT-181, found via a scheduled improvement hunt) wraps the 2-line "call `_record_message_safe`, cap `$offset_cap` on failure" pattern that appeared identically at all 5 call sites above - extracted into one helper taking `$offset_cap` by scalar ref, since the value must persist and combine across multiple calls within one `run_once` invocation. Pure refactor, no behavior change. `cli/poller.pl`'s own `D2TG::Store->new(...)` startup call (TGT-183, found via a scheduled hourly bug hunt, reproduced live) was unwrapped - a storage-open failure at that specific step crashed the poller with a raw, uncaught Perl exception (potentially embedding the real db path) instead of the same clean, scrubbed refusal `require_existing_base_dir`/`D2TG::Lock::acquire` already produce for their own failures. Now wrapped and classified via `_classify_store_error`, matching TGT-133's own established scrubbing precedent. Two earlier startup steps (`lock_path`/`heartbeat_path`, both of which internally call `make_path` on the same `.tira` directory) shared the identical unwrapped risk - fixed separately as TGT-184 (both now eval-wrapped and scrubbed the same way; `lock_path`'s own failure is reproduced live and tested, `heartbeat_path` wrapped identically - its own failure isn't independently exercised by the current test's static filesystem setup, though an external filesystem change between the two calls could still make it fail, not because it's impossible to test, just not covered by this pass; a Codex QA-stage review on TGT-184 also found the `heartbeat_path` failure branch leaked the just-acquired startup lock file by exiting before `D2TG::Lock::release` - fixed by releasing it first; two more pre-existing exit paths sharing that same lock-leak gap were found in the same review and filed separately as TGT-185). TGT-185 (shipped in 1.50) fixed the `D2TG::Store->new` failure exit's own identical lock leak the same way; its other originally-scoped scenario (a no-groups-configured exit also leaking the lock) was suspected of sharing the gap too but turned out to be unreachable dead code - two earlier startup guards already refuse before that check can ever see an empty groups list. A second Codex QA-stage review round on TGT-185's own fix then found two MORE reachable leaking exits (a "no bot tokens configured" exit, and the exec()-restart-failure die further down) - rather than patching a fourth site individually, `cli/poller.pl` now has a single `END` block right after `D2TG::Lock::acquire` succeeds that releases the lock on every Perl-managed `exit`/uncaught `die` past that point - standard Perl `END`-block behavior, it does not run if the process terminates on an untrapped signal (`SIGKILL` always, or any signal without an installed handler at that moment); no claim made about exactly when `SIGTERM`/`SIGINT` become safe; `D2TG::Lock`'s own staleness/eviction logic recovers a lock left behind that way on the next poller start, not automatically - a structural fix for the whole bug class; `t/185-poller-lock-leak-no-groups-and-store-failure.t` proves the no-bot-tokens scenario and documents why the exec()-failure one isn't independently tested. `open_store_or_die` (TGT-186, found via a scheduled JOB-003 hourly bug hunt, reproduced live against `cli/history.pl`) is a new shared helper wrapping `D2TG::Store->new` in `eval`, classifying via `_classify_store_error` and printing the same scrubbed `Failed to open local storage (REASON) - refusing to start.` refusal `cli/poller.pl`'s own TGT-183 fix established, exiting 1 on failure - used by 7 `cli/*.pl` scripts (`attachment`, `text-only-replies`, `approve`, `retry-download`, `history`, `reply`, `unread`) that each independently constructed the identical unwrapped call before this fix; `cli/poller.pl`'s own inline version is left as-is since its TGT-185 lock-release logic is intertwined with that specific call site. `persist_offset_safe`'s new true/false return-value contract (TGT-191, a live production incident: 2 real Telegram messages permanently lost) closes a genuine offset-advance-vs-persist race - `cli/poller.pl`'s main loop used to advance its in-memory offset unconditionally regardless of whether the write actually landed, so a still-running process's own next `getUpdates` call could confirm an unpersisted batch to Telegram (which never redelivers an update once a later offset is sent); a crash before a later persist caught up then lost that batch forever. The main loop now only advances on a true return; `t/191-offset-not-advanced-on-persist-failure.t` simulates a persist-fails-then-succeeds scenario proving both the held-back offset and the eventual redelivery. The 4 `STORE ERROR` print blocks for `is_allowed`/`add_pending` (TGT-193, found via a scheduled JOB-004 improvement hunt) previously echoed the raw DBI/SQLite exception text verbatim - these call sites predate `_classify_store_error` (TGT-165, before TGT-167 extracted the shared helper) and were never revisited to match; now classified the same way every other `D2TG::Store` error path in this module already is (`is_allowed` is a read, not a write, but is still classified the same way). A full sweep of this module confirmed these were the only 4 remaining `D2TG::Store`-specific raw-echo sites - the module's other 3 remaining `my $error = $@; ... print STDERR ...$error...` patterns (`run_once_safe`'s generic `POLL ERROR`, `skill_version_check_safe`'s `.env`-read error, `_run_non_fatal`'s media download/transcription error) are each a different, unrelated error category, deliberately out of this ticket's own scope. New public `store_write_safe($chat_id, $description, $coderef)` (TGT-198, found via a scheduled JOB-004 improvement hunt) - promoted from `D2TG::Reply`'s own private `_store_write_safe` (TGT-192) - runs `$coderef->()` in `eval`, classifies any failure via `_classify_store_error`, prints the same `STORE ERROR [chat_id]: DESC failed - REASON` line, and returns a `($ok, $value)` pair (`$ok` false on failure, `$value` the coderef's own return value otherwise) so a caller needing the result (e.g. `is_allowed`) can still distinguish "the write failed" from "the write succeeded and returned false". Migrated the 4 identically-shaped `is_allowed`/`add_pending` call sites in this module and 3 more in `D2TG::Download` to call it instead of hand-duplicating the pattern - a pure refactor, no behavior change. `_record_message_safe`/`persist_offset_safe` were deliberately left unmigrated (their own printed text and 0/1 return-boolean contract differ from this shape). `run_once` (TGT-204, a real live-reported incident) now also prints a `NEW TG MEDIA FAILED [chat_id] sender: media_kind - queued for retry, RETRY WITH: d2 tg.retry-download --all` stdout line whenever a media download fails and the failure is successfully queued via `record_failed_download` - previously the only signal was the `MEDIA DOWNLOAD ERROR` line to STDERR, which never reaches the monitor job's own stdout-fed `tira.policy.bridge` notification stream, leaving a queued failure invisible until a human/agent thought to check speculatively. Printed only when the queue write itself succeeds (a failure to queue keeps its own existing STDERR-only report unchanged). The masked `--bot <token>` flag fragment printed on both `_print_reply_template`'s own `REPLY WITH` line and the `NEW TG MEDIA FAILED` branch's `RETRY WITH` line (TGT-220) is now built by a single shared `_bot_flag($bot_token)` helper (TGT-226, found via a scheduled JOB-004 improvement hunt) instead of an identical ternary duplicated at both call sites - a pure consolidation, no behavior change; both templates' printed output is byte-for-byte unchanged. `_print_reply_template`'s own `REPLY WITH` line (TGT-227, found via a scheduled JOB-003 hourly bug hunt, a security-relevant fix) now prints `--bot` BEFORE `chat_id` instead of after - the trailing position it previously used was never recognized as a flag by `cli/reply.pl`'s own parser (leading-only) or `D2TG::Reply::parse_cli_args` (no trailing `--bot` handling), so it fell into the reply text itself, leaking the real token into the sent message when substituted in place and silently sending from the wrong bot; the `RETRY WITH` line (a different command, `d2 tg.retry-download`, with its own leading-only `--bot` parsing already correct) was unaffected. The `edited_message` branch (TGT-217, found via a scheduled JOB-003 hourly bug hunt) now also calls `_print_reply_template` right after its own `NEW TG EDIT` line, for both the text-edit and caption/media-only-edit cases - every other actionable branch already did this, and this was the sole one missing it, leaving an edited message announced with no ready-to-run reply command for the monitoring agent. The `record_failed_download` call (TGT-219, found via a scheduled JOB-004 improvement hunt) now also passes its own `$bot_token` through as `bot_key` - previously always dropped, so a media-download failure in a multi-bot config was queued unscoped, and Telegram's own bot-token-scoped `file_id` values meant a retry could silently use the wrong bot. The `NEW TG MEDIA FAILED` line's own `RETRY WITH` hint (TGT-220, found via a scheduled JOB-003 hourly bug hunt) now also appends a masked `--bot <token>` flag whenever `$bot_token` is defined, matching `_print_reply_template`'s own already-established convention - previously it was a hard-coded literal with no `--bot` flag even though `$bot_token` was already in scope at that exact print site (used for the very `record_failed_download` call above), so following it literally for a non-default-bot failure in a multi-bot config retried nothing, since `cli/retry-download.pl --all` with no `--bot` only acts on the default-bot sentinel queue. Single-bot mode (no `$bot_token`) is unchanged. `_display_name`/`_reply_context_suffix`/`_timestamp_prefix`/`_sanitize_for_stdout`/`_bot_flag`/`_print_reply_template`/`_print_attachment_template`/`_reaction_key`/`_reaction_label`/`_format_forwarded_sender`/`_media_kind` (TGT-259, found via a scheduled JOB-005... TGT-258 module-decomposition survey) are now thin forwarders onto `D2TG::Poller::Format` - no behavior/signature change for any caller. `_stored_summary`/`_forward_origin_name` were removed entirely (not forwarded): coverage proved their only caller was another sub in the same extracted cluster, now calling `D2TG::Poller::Format`'s own bare functions directly - keeping a forwarder for either would have been permanently-uncallable dead code. |
| `D2TG::Poller::Format` | The 13-sub stdout-line formatting/presentation cluster extracted out of `D2TG::Poller` (TGT-259, ~244 lines, no poll-loop state - pure functions of their inputs). Implements `display_name`, `reply_context_suffix`, `stored_summary`, `timestamp_prefix`, `sanitize_for_stdout`, `bot_flag`, `print_reply_template`, `print_attachment_template`, `reaction_key`, `reaction_label`, `forward_origin_name`, `format_forwarded_sender`, `media_kind` - unchanged in behavior from their pre-extraction selves. `D2TG::Poller` is the only caller in practice, via its own forwarders (11 of the 13 - see that module's own entry for the 2 that got none). |
