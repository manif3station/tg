...
improvement-hunt fixing a hand-rolled duplicate) - a pure consistency
fix, not a change in what invocations it accepts.

`--bot <token>` (TGT-340, found via a live, user-requested adversarial
improvement hunt) reports that specific token's own masked value
instead of `D2TG_TOKEN`'s - matching `D2TG::Reply::Args::extract_bot_flag`'s
established leading-position convention, already used by nearly every
other `d2 tg.*` command that can scope to a specific bot. Before this,
a multi-bot install (TGT-049) had no way to confirm a non-default
bot's own token via this command. `chat_id` is unaffected either way,
since it is a single global env var, not a per-bot value. Omitting
`--bot` is byte-for-byte unchanged from before this ticket.
