...
(C<AUTO_RETRY_WINDOW_SECONDS> since queued) and not retried in the last
C<AUTO_RETRY_INTERVAL_SECONDS>, optionally filtered by C<bot_key>.
...
Mirrors L<record_failed_download()|/record_failed_download($chat_id, $message_id, $file_id, %opts)> exactly, for the C<failed_transcriptions>
table. C<%opts>: C<sender>, C<error>, C<bot_key>.

=head2 failed_transcriptions(%opts)

Mirrors L<failed_downloads()|/failed_downloads(%opts)>.

=head2 failed_transcriptions_due_for_retry(%opts)

Mirrors L<failed_downloads_due_for_retry()|/failed_downloads_due_for_retry(%opts)>.

=head2 mark_failed_transcription_retried($id)

Mirrors L<mark_failed_download_retried()|/mark_failed_download_retried($id)>.

=head2 remove_failed_transcription($id)

Mirrors L<remove_failed_download()|/remove_failed_download($id)>.
...
exactly (TGT-196) - persists a queued row's own successfully-produced
transcript, so a future retry
(C<D2TG::Transcribe::Retry::retry_failed_transcription>) can skip
re-downloading and re-transcribing entirely and retry only the failing
C<record_message> write, instead of re-running whisper (the single most
expensive step in this pipeline) on every retry cycle.
...
TGT-330 (found via a live JOB-004 improvement hunt): C<has_failed_
download>/C<has_failed_transcription> (TGT-270) joined this same
pattern - both now delegate to a sixth private, table-parameterized
helper, C<_has_failed($table, $chat_id, $message_id, %args)>. Pure
extraction, no behavior change.

TGT-333 (found via a live JOB-004 improvement hunt): the C<failed_
transcriptions> table gained its own C<transcript> column, and
C<mark_failed_transcription_transcribed> was added to give it the same
skip-already-done escape hatch C<local_path>/
C<mark_failed_download_downloaded> already gave C<failed_downloads>
(TGT-196) - this was the one asymmetry left between the two otherwise
fully-mirrored queues.
