...
D2TG::Store::History - message-history storage for the tg skill

=head1 SYNOPSIS

    my $history = D2TG::Store::History->new( dbh => $dbh );
    $history->record_message( $chat_id, $message_id, $sender, $summary );

=head1 DESCRIPTION

TGT-278 (found via a wc -l sweep run as TGT-277's own pipeline-
continuity backlog check): D2TG::Store.pm was 1378 lines - far over
the board's 500-line-per-module cap. The 8 message-history methods
below - record_message/get_message/get_attachment_path/mark_read/
is_read/unread_messages/recent_messages/messages_in_range - form the
largest single cohesive cluster in that module (they only need the
shared C<$dbh>, not any of D2TG::Store's own access-control/offset/
retry-queue state), so they were relocated here first, mirroring
D2TG::Store::RetryQueue's own established precedent (TGT-257):
built once in D2TG::Store::new, reused by every forwarding method
D2TG::Store itself keeps, no behavior change for any existing caller.

=head1 METHODS

=head2 _select_all_rows($self, $sql, @bind)

TGT-324 (found via a live JOB-004 improvement hunt):
L<unread_messages()|/unread_messages(bot_key =E<gt> $optional_bot_key)>,
L<recent_messages()|/recent_messages($limit = 10, bot_key =E<gt> $optional_bot_key)>,
and L<messages_in_range()|/messages_in_range(since =E<gt> $iso8601, until =E<gt> $iso8601, bot_key =E<gt> $optional_bot_key)>
each independently built their own C<$sql>/C<@bind>, but all three
ended with the identical 2 lines - running C<selectall_arrayref> and
returning the dereferenced list. Collapsed into this one private
helper; each caller now passes its own C<$sql>/C<@bind> and gets back
the same list it always did. Pure extraction, no behavior change.
...
in a separate column instead, reachable only via L<get_attachment_path()|/get_attachment_path($chat_id, $message_id, bot_key =E<gt> $optional_bot_key)>,
...
C<local_path> - see L<get_attachment_path()|/get_attachment_path($chat_id, $message_id, bot_key =E<gt> $optional_bot_key)>. C<bot_key> (TGT-232)
...
same-second tiebreaker as L<unread_messages()|/unread_messages(bot_key =E<gt> $optional_bot_key)> (TGT-075). Either bound
