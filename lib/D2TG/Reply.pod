...
TGT-308 (found via a JOB-003 hourly bug hunt): the
C<is_recent_duplicate_reply> call itself is C<eval>-wrapped and
classified via C<D2TG::Poller::Safe::classify_store_error> - a
locked/busy database here used to die raw, and since
C<L</format_send_error($error)>> prints its caught exception
verbatim, that raw text (which can embed the real db path) used to
reach a user's own STDERR directly via C<cli/reply.pl>'s normal error
output, not merely crash silently.
...
shared predicate, also used by C<D2TG::Poller::Safe::run_once_safe>), in which
