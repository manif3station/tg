...
D2TG::Config::Flags - CLI flag-parsing helpers (--db/-d, --chat_id/--bot groups)

=head1 SYNOPSIS

    my $value = D2TG::Config::Flags::shift_flag_value( \@args, '--flag' );
    my ( $alias, @rest ) = D2TG::Config::Flags::extract_db_flag(@ARGV);
    my ( $alias, @rest ) = D2TG::Config::Flags::extract_db_flag_or_die(@ARGV);
    my ( $groups, @rest ) = D2TG::Config::Flags::bot_groups( argv => \@ARGV );

=head1 DESCRIPTION

TGT-267: extracted out of L<D2TG::Config> (which owned these four
functions since TGT-049/TGT-051/TGT-072/TGT-177) into their own module
- CLI flag-parsing is a distinct concern from L<D2TG::Config>'s own
env-reading/version/error-classification concerns. Mirrors
L<D2TG::Reply::Args>/L<D2TG::Config::Paths>'s own precedent.
...
Used by L<extract_db_flag()|/extract_db_flag(@ARGV)>, L<bot_groups()|/bot_groups(argv =E<gt> \@argv, env_chat_id =E<gt> $id, env_token =E<gt> $token)>'s C<--chat_id> handling,
...
=head2 shift_flag_value_free_text($args_arrayref, $flag_label)

TGT-332 (found via a live JOB-003 hourly bug hunt; Q-020 answered by
Michael 2026-09-22): a sibling of L<shift_flag_value()|/shift_flag_value($args_arrayref, $flag_label)> for flags whose
value is genuinely free text rather than a token/id - currently only
C<cli/send.pl>'s C<--caption>. Shifts the next value off
C<$args_arrayref> and dies with C<"$flag_label requires a value\n">
unless it's defined and non-empty, exactly like C<shift_flag_value>'s
own undef/empty guard - but deliberately omits the flag-shape check
(rejecting a value starting with C<-> followed by a letter), since a
caption that legitimately starts with C<--> (e.g. C<"--dry-run flag
explained">) is real content, not a mistake, and must never be misread
as a missing flag value the way C<--bot>/C<--db>/
C<--reply-to-message-id> correctly are.
...
through L<extract_db_flag_or_die()|/extract_db_flag_or_die(@ARGV)> rather than calling this function
...
L<extract_db_flag_or_die()|/extract_db_flag_or_die(@ARGV)> instead of wrapping this call in its own
...
L<shift_flag_value()|/shift_flag_value($args_arrayref, $flag_label)> (TGT-072), which also backs L<bot_groups()|/bot_groups(argv =E<gt> \@argv, env_chat_id =E<gt> $id, env_token =E<gt> $token)>'s
...
TGT-177 (found via a scheduled improvement hunt): wraps L<extract_db_flag()|/extract_db_flag(@ARGV)>
- the same idiom 10 C<cli/*.pl> scripts (C<approve>/C<attachment>/C<history>/
...
L<D2TG::Config::Paths/resolve_alias_dir_or_die>'s own identical TGT-172
extraction of the same shape for C<resolve_alias_dir>. A one-line
forwarder onto the shared L<D2TG::OrDie/or_die> helper (TGT-269, found
via a scheduled JOB-004 improvement hunt: this same eval/print-STDERR/
exit(1) wrapper shape was found duplicated a further 3 times across
other modules). Pure refactor: same exit code, same STDERR text
(byte-for-byte, not just the same wording), same return shape
C<($alias, @remaining_args)> - no caller's own observable behavior
changes. C<cli/reply.pl> is not among the
callers, since it uses its own separate leading-position-only C<--db>
extraction (see L<extract_db_flag()|/extract_db_flag(@ARGV)>'s own POD above) rather than this
function.
...
L<shift_flag_value()|/shift_flag_value($args_arrayref, $flag_label)>, same as C<--chat_id>'s own validation above.
...
This validation is now delegated to L<shift_flag_value()|/shift_flag_value($args_arrayref, $flag_label)> (TGT-072),
...
channel), reusing the identical check L<D2TG::Config/require_chat_id_or_warn>
