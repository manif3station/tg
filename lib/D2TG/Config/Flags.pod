...
extraction of the same shape for C<resolve_alias_dir>. Pure refactor:
same exit code, same STDERR text (byte-for-byte, not just the same
wording), same return shape C<($alias, @remaining_args)> - no caller's
own observable behavior changes. C<cli/reply.pl> is not among the
