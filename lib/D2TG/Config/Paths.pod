...
TGT-287 (found via a scheduled JOB-003/004 sweep): this module's own
embedded POD (this file, previously kept inline after C<__END__>) was
extracted to this separate F<.pod> file at the same time the new
C<resolve_and_require_base_dir_or_die> function was added, matching
this project's established POD-in-a-separate-file convention.
...
L<resolve_alias_dir()|/resolve_alias_dir(alias =E<gt> $a, tira_home =E<gt> $t, paths =E<gt> $p)>,
printing to STDERR and exiting 1 on failure instead of propagating the
die - a one-line forwarder onto the shared L<D2TG::OrDie/or_die> helper
(TGT-269).
...
L</require_existing_base_dir($base_dir)>, printing to STDERR and
exiting 1 on failure instead of propagating the die - a one-line
forwarder onto the shared L<D2TG::OrDie/or_die> helper (TGT-269).

=head2 resolve_and_require_base_dir_or_die(alias => $a, tira_home => $t, paths => $p)

TGT-287: composes L</resolve_alias_dir_or_die(%args)> immediately
followed by L</require_existing_base_dir_or_die($base_dir)> on its
result - the exact pairing every C<cli/*.pl> caller that opens storage
already needs, previously hand-copied as two separate calls across 12
scripts.
