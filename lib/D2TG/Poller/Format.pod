...
genuine bug): the C<get_message> call itself is C<eval>-wrapped - a
locked/busy database used to die raw here, the same
raw-crash/db-path-leak risk TGT-183/186/195/293 already fixed
elsewhere. A failure now returns C<undef>, identical to a legitimate
"nothing stored" result, letting L</reply_context_suffix($message,
$store, $chat_id, $bot_token)>'s own existing no-lookup fallback take
over instead of propagating the exception.
...
a single stdout line stays single-line. Strips both the 7-bit control
ranges (C<\x00-\x08>, C<\x0B-\x1F> including ESC C<\x1B>, and DEL
C<\x7F>) and the C1 control range (C<\x80-\x9F>, TGT-326 - found via a
live JOB-003 hourly bug hunt) - C<\x9B>, the 8-bit form of CSI (Control
Sequence Introducer), triggers the identical terminal-escape-sequence
class as ESC+C<[> in 7-bit form, and was reachable via arbitrary
Telegram message text before this fix.
...
=head2 print_fetch_template($chat_id, $message_id)

TGT-311 (explicit user-requested architecture change): prints the
C<FETCH WITH: d2 tg.fetch ...> stdout line - the command that reveals a
new text or successfully-transcribed-voice message's own content
(never printed inline anymore) and marks it read as a side effect.
