...
called directly, for the identical reason
L<D2TG::Poller::Safe/record_message_safe> wraps C<record_message>
(TGT-132): both run against a
...
L<D2TG::Poller::Safe/run_once_safe> preserves the pre-batch offset on error, causing the
...
TGT-273 (found via a scheduled JOB-004 improvement hunt, a direct
follow-up sweep after TGT-270 hardened the plain-message/media/voice
branch against a Telegram redelivery): when the edit carries text, the
incoming sanitized text is compared against C<$store>'s already-stored
C<summary> for that C<(chat_id, message_id)> B<before> printing
C<NEW TG EDIT> - identical means this exact edit was already announced
on a prior cycle (a redelivery, skip entirely); different or no stored
row at all means a genuinely new edit, or the first one ever seen
(announce and record as before). Reusing C<get_message> presence alone
the way TGT-270 did for the plain-message branch does NOT work here:
C<record_message> upserts on C<(chat_id, bot_key, message_id)>, so
C<get_message> is already non-null for ANY message ever recorded,
including the ORIGINAL pre-edit send - a presence-only check would
wrongly suppress a genuinely new edit the very first time it arrived. A
transient lookup error degrades the same way every other dedup check in
this module already does (treated as not-previously-seen, proceed as
normal). Scoped to the C<has_text> case only - a caption/media-only
edit is never recorded via C<record_message> at all (see above), so
there is no stored state to compare against; a redelivered caption-only
edit still re-announces, an accepted, documented gap. Also prints a
C<REPLY WITH> template (TGT-217,
...
L<D2TG::Store>-shaped object. Returns the raw updates array and the
next offset to pass on the following call.

TGT-276 (filed via TGT-275's own REQ-029 audit): C<run_once> itself is
now a short dispatch loop - for each update in the batch, it calls one
of L<D2TG::Poller::Dispatch>'s 3 handler functions
(C<handle_message_reaction>, C<handle_edited_message>, or
C<handle_plain_update>, in that priority order) and moves on to the
next update. The full behavior of each update type (access control,
what gets printed, what gets recorded, redelivery-dedup, failure
queuing) lives in L<D2TG::Poller::Dispatch>'s own POD, not here - the
extensive per-branch history that used to live in this section was
moved there along with the code itself, matching the same
POD-follows-the-code convention this project already used for
L<D2TG::Poller::Safe>.

The one piece of behavior that stays in C<run_once> itself, because it
spans the whole batch rather than any single update: B<TGT-178>
(Michael's own architectural ruling, answering the message-loss
investigation raised as TGT-176) - a handler's own store-write failure
is non-fatal (see L<D2TG::Poller::Safe/record_message_and_track_offset>),
but C<run_once> tracks the first update whose call failed, in the order
the batch is iterated (the same as the earliest, since Telegram
delivers updates in increasing C<update_id> order), and at the end of
the batch caps the returned offset at that failing update's own
C<update_id> instead of the batch's full next offset - Telegram never
redelivers an update once the offset has moved past it, so returning
the batch's full offset regardless would let a genuine store-write
failure permanently and silently lose that message's local history.

This module's own embedded POD was extracted into C<Poller.pod> (this
file, TGT-273). C<Poller.pm> is now 74 lines (TGT-276 relocated
C<run_once>'s own 3 branch handlers into L<D2TG::Poller::Dispatch>,
after TGT-275 had already relocated the 8 non-C<run_once> helper
functions into L<D2TG::Poller::Safe>) - both comfortably under the
board's 500-line-per-module cap.
