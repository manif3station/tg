...
touched, via the shared L<resolve_alias_dir_or_die()|/resolve_alias_dir_or_die(alias =E<gt> $alias, paths =E<gt> \%paths)> wrapper (TGT-172,
...
directory it returns actually exists - see
L<require_existing_base_dir()|/require_existing_base_dir($base_dir)>
...
L<resolve_alias_dir()|/resolve_alias_dir(alias =E<gt> $alias, paths =E<gt> \%paths)> that catches its die, prints the message to
...
script calls this immediately after
L<resolve_alias_dir()|/resolve_alias_dir(alias =E<gt> $alias, paths =E<gt> \%paths)> returns, so a
...
wrapper around L<require_existing_base_dir()|/require_existing_base_dir($base_dir)> that catches its die,
...
matching L<resolve_alias_dir_or_die()|/resolve_alias_dir_or_die(alias =E<gt> $alias, paths =E<gt> \%paths)>'s own established shape for the
...
same way L<skill_version()|/skill_version(default_root =E<gt> $path)> does), or C<undef> if C<Changes> can't be
...
L<D2TG::Poller::Safe/skill_version_check_safe>/
L<D2TG::Poller::Safe/persist_offset_safe>).
...
L<skill_version()|/skill_version(default_root =E<gt> $path)>) uses this instead of blindly C<exec()>ing the literal
...
Mirrors L<lock_path()|/lock_path(default_root =E<gt> $path, base_dir =E<gt> $path)>'s own resolution exactly (TGT-116): given a
...
L<lock_path()|/lock_path(default_root =E<gt> $path, base_dir =E<gt> $path)> itself uses for C<telegram.pid>/C<poller.pid>.
...
timestamp was written - a concurrent L<heartbeat_age()|/heartbeat_age($path)> read (from C<d2
...
L<write_heartbeat()|/write_heartbeat($path, renamer =E<gt> \&coderef)>, or C<undef> if the file doesn't exist or doesn't
...
C<D2TG::Poller::Safe::run_once_safe> (TGT-097) uses it to decide whether a
