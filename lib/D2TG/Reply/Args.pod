...
send/voice/store-write concern, and L</extract_bot_flag_or_die> is
...
L</shift_flag_value>'s own underlying call already dies correctly when
...
C<or_die> calls L</extract_bot_flag> in an C<eval>; on failure, prints
...
bare-C<--bot> bug TGT-264 already fixed inside L</extract_bot_flag>
itself). L</extract_bot_flag> itself is unchanged.
