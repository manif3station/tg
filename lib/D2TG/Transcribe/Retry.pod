...
which L</retry_failed_transcription> (TGT-237) never got despite being
...
Deliberately does NOT branch on L</retry_failed_transcription>'s own
C<($ok, $result)> return value to decide whether to stamp
C<last_retry_at> - unlike L<D2TG::Download/retry_failed_download>,
L</retry_failed_transcription> always returns C<(1, $transcript)> once
...
change to L</retry_failed_transcription>'s own existing return contract.
