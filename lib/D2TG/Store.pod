...
(see C<_seed_admin> and L<is_allowed()|/is_allowed($chat_id, $bot_key)>) so the admin is auto-approved
...
its own C<bot_key> (matching L<pending_chat_ids()|/pending_chat_ids(bot_key =E<gt> $bot_key)>' own established
...
the reply's own text, stored so L<is_recent_duplicate_reply()|/is_recent_duplicate_reply($chat_id, $text, bot_key =E<gt> $key, window_seconds =E<gt> $n)> can check
it later.
